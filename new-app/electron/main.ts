...
async function checkKeyboards() {
  try {
    ...
    return {
      ...
      keyboards: keyboards.map((kb: UsbDevice) => kb.name || 'Unknown Keyboard')
    }
  } catch (error) {
...
async function checkInterviewCoder() {
  try {
    ...
    // Define known interview coder applications and their process names
    ...
    const interviewCoderProcesses = processes.filter(process => {
      ...
      return interviewCoderApps.some(app => 
        name.includes(app.toLowerCase()) || 
        cmd.includes(app.toLowerCase())
      )
    })
